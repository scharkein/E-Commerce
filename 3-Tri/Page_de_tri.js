...
function updateMinPrice() {
  let checkedCheckboxestype = document.querySelectorAll(
...
function filterCombined() {
  ...
  const maxPrice = parseFloat(maxVal.value);
  // type
  const checkedCheckboxestype = document.querySelectorAll(
    'input[name="type"]:checked'
  );
  // carte graphique
  const checkedCheckboxesVram = document.querySelectorAll(
    'input[name ="Vram"]:checked'
  );
  const checkedCheckboxesCore = document.querySelectorAll(
    'input[name ="Core"]:checked'
  ); //lui
  const checkedCheckboxesC_carte_graphique = document.querySelectorAll(
    'input[name ="C_carte_graphique"]:checked'
  ); //lui
  const checkedCheckboxesRGB = document.querySelectorAll(
    'input[name ="rgb"]:checked'
  );
  // processeur
  const checkedCheckboxesCoeurs = document.querySelectorAll(
    'input[name ="Coeurs"]:checked'
  );
  const checkedCheckboxesf_processeur = document.querySelectorAll(
    'input[name ="f_processeur"]:checked'
  ); //lui
  const checkedCheckboxesC_processeurs = document.querySelectorAll(
    'input[name ="C_processeurs"]:checked'
  ); //lui
  // RAM
  const checkedCheckboxesR_type = document.querySelectorAll(
    'input[name ="R_type"]:checked'
  );
  const checkedCheckboxesf_RAM = document.querySelectorAll(
    'input[name ="f_RAM"]:checked'
  ); // lui
  const checkedCheckboxesR_stockage = document.querySelectorAll(
    'input[name ="R_stockage"]:checked'
  );
  // carte mere
  const checkedCheckboxesC_type = document.querySelectorAll(
    'input[name ="C_type"]:checked'
  );
  const checkedCheckboxesf_carte_mere = document.querySelectorAll(
    'input[name ="f_carte_mere"]:checked'
  ); //lui
  const checkedCheckboxesC_format = document.querySelectorAll(
    'input[name ="C_format"]:checked'
  );
  //stockage
  const checkedCheckboxesS_taille = document.querySelectorAll(
    'input[name ="S_Taille"]:checked'
  );
  const checkedCheckboxesf_stockage = document.querySelectorAll(
    'input[name ="f_stockage"]:checked'
  ); //lui
  const checkedCheckboxesS_type = document.querySelectorAll(
    'input[name ="S_type"]:checked'
  );
  // boitier
  const checkedCheckboxesB_dimension = document.querySelectorAll(
    'input[name ="B_dimension"]:checked'
  );
  const checkedCheckboxestaille_carte_mere = document.querySelectorAll(
    'input[name ="taille_carte_mere"]:checked'
  );
  const checkedCheckboxestaille_alimentation = document.querySelectorAll(
    'input[name ="taille_alimentation"]:checked'
  );
  const checkedCheckboxestaille_carte_graphique = document.querySelectorAll(
    'input[name ="taille_carte_graphique"]:checked'
  );
  // alimentation
  const checkedCheckboxesA_dimension = document.querySelectorAll(
    'input[name ="A_dimension"]:checked'
  );
  const checkedCheckboxesC_alimentation = document.querySelectorAll(
    'input[name ="C_alimentation"]:checked'
  );
  const checkedCheckboxesA_type = document.querySelectorAll(
    'input[name ="A_type"]:checked'
  );
  // refroidissement
  const checkedCheckboxesRef_type = document.querySelectorAll(
    'input[name ="Ref_type"]:checked'
  );
  const checkedCheckboxesDebit_dair = document.querySelectorAll(
    'input[name ="Debit_dair"]:checked'
  );
  const checkedCheckboxessocket = document.querySelectorAll(
    'input[name ="socket"]:checked'
  );
  const checkedCheckboxesventilateur = document.querySelectorAll(
    'input[name ="ventilateur"]:checked'
  );
  ...
  const isS_typeCheckboxChecked = checkedCheckboxesS_type.length > 0;
  const isB_dimensionCheckboxChecked = checkedCheckboxesB_dimension.length > 0;
  const istaille_carte_mereCheckboxChecked =
    checkedCheckboxestaille_carte_mere.length > 0;
  const istaille_alimentationCheckboxChecked =
    checkedCheckboxestaille_alimentation.length > 0;
  const istaille_carte_graphiqueCheckboxChecked =
    checkedCheckboxestaille_carte_graphique.length > 0;
  const isA_dimensionCheckboxChecked = checkedCheckboxesA_dimension.length > 0;
  const isC_alimentationCheckboxChecked =
    checkedCheckboxesC_alimentation.length > 0;
  const isA_typeCheckboxChecked = checkedCheckboxesA_type.length > 0;
  const isRef_typeCheckboxChecked = checkedCheckboxesRef_type.length > 0;
  const isDebit_dairCheckboxChecked = checkedCheckboxesDebit_dair.length > 0;
  const issocketCheckboxChecked = checkedCheckboxessocket.length > 0;
  const isCoreCheckboxChecked = checkedCheckboxesCore.length > 0;
  const isventilateurCheckboxChecked = checkedCheckboxesventilateur.length > 0;
  const isC_carte_graphiqueCheckboxChecked =
    checkedCheckboxesC_carte_graphique.length > 0;
  const isf_processeurCheckboxChecked =
    checkedCheckboxesf_processeur.length > 0;
  const isf_RAMCheckboxChecked = checkedCheckboxesf_RAM.length > 0;
  const isC_processeursCheckboxChecked =
    checkedCheckboxesC_processeurs.length > 0;
  const isf_carte_mereCheckboxChecked =
    checkedCheckboxesf_carte_mere.length > 0;
  const isf_stockageCheckboxChecked = checkedCheckboxesf_stockage.length > 0;

  // Filtrer les articles en fonction des catégories, du prix ... (peut etre améliorer par crit1,2 .... mais prefere)
  document.querySelectorAll(".card").forEach((article) => {
    //generale
    const type = article.dataset.type;
    const prixArticle = parseFloat(article.dataset.prix);
    ...
    const Vram = article.dataset.crit1;
    const Core = article.dataset.crit2;
    const C_carte_graphique = article.dataset.crit3;
    const RGB = article.dataset.crit4;
    ...
    //generale
    const isCategoryMatched = isTypeChecked
      ? [...checkedCheckboxestype].some((cb) => cb.id === type)
      : true;
    const isPriceInRange = prixArticle >= minPrice && prixArticle <= maxPrice;
    //carte graphique
    const isVramMatched = isVramCheckboxChecked
      ? [...checkedCheckboxesVram].some((cb) => cb.id === Vram)
      : true;
    const isCoreMatched = isCoreCheckboxChecked
      ? [...checkedCheckboxesCore].some((cb) => cb.id === Core)
      : true;
    const isC_carte_graphiqueMatched = isC_carte_graphiqueCheckboxChecked
      ? [...checkedCheckboxesC_carte_graphique].some(
          (cb) => cb.id === C_carte_graphique
        )
      : true;
    const isRgbMatched = isRgbCheckboxChecked
      ? [...checkedCheckboxesRGB].some((cb) => cb.id === RGB)
      : true;
    //processeur
    const isCoeursMatched = isCoeursCheckboxChecked
      ? [...checkedCheckboxesCoeurs].some((cb) => cb.id === Coeurs)
      : true;
    const isf_processeurMatched = isf_processeurCheckboxChecked
      ? [...checkedCheckboxesf_processeur].some((cb) => cb.id === f_processeur)
      : true;
    const isC_processeursMatched = isC_processeursCheckboxChecked
      ? [...checkedCheckboxesC_processeurs].some(
          (cb) => cb.id === C_processeurs
        )
      : true;
    //RAM
    const isR_typeMatched = isR_typeCheckboxChecked
      ? [...checkedCheckboxesR_type].some((cb) => cb.id === R_type)
      : true;
    const isf_RAMMatched = isf_RAMCheckboxChecked
      ? [...checkedCheckboxesf_RAM].some((cb) => cb.id === f_RAM)
      : true;
    const isR_stockageMatched = isR_stockageCheckboxChecked
      ? [...checkedCheckboxesR_stockage].some((cb) => cb.id === R_stockage)
      : true;
    //carte mere
    const isC_typeMatched = isC_typeCheckboxChecked
      ? [...checkedCheckboxesC_type].some((cb) => cb.id === C_type)
      : true;
    const isf_carte_mereMatched = isf_carte_mereCheckboxChecked
      ? [...checkedCheckboxesf_carte_mere].some((cb) => cb.id === f_carte_mere)
      : true;
    const isC_formatMatched = isC_formatCheckboxChecked
      ? [...checkedCheckboxesC_format].some((cb) => cb.id === C_format)
      : true;
    // stockage
    const isS_tailleMatched = isS_tailleCheckboxChecked
      ? [...checkedCheckboxesS_taille].some((cb) => cb.id === S_taille)
      : true;
    const isf_stockageMatched = isf_stockageCheckboxChecked
      ? [...checkedCheckboxesf_stockage].some((cb) => cb.id === f_stockage)
      : true;
    const isS_typeMatched = isS_typeCheckboxChecked
      ? [...checkedCheckboxesS_type].some((cb) => cb.id === S_type)
      : true;
    // boitier
    const isB_dimensionMatched = isB_dimensionCheckboxChecked
      ? [...checkedCheckboxesB_dimension].some((cb) => cb.id === B_dimension)
      : true;
    const istaille_carte_mereMatched = istaille_carte_mereCheckboxChecked
      ? [...checkedCheckboxestaille_carte_mere].some(
          (cb) => cb.id === taille_carte_mere
        )
      : true;
    const istaille_alimentationMatched = istaille_alimentationCheckboxChecked
      ? [...checkedCheckboxestaille_alimentation].some(
          (cb) => cb.id === taille_alimentation
        )
      : true;
    const istaille_carte_graphiqueMatched =
      istaille_carte_graphiqueCheckboxChecked
        ? [...checkedCheckboxestaille_carte_graphique].some(
            (cb) => cb.id === taille_carte_graphique
          )
        : true;
    // alimentation
    const isA_dimentionMatched = isA_dimensionCheckboxChecked
      ? [...checkedCheckboxesA_dimension].some((cb) => cb.id === A_dimension)
      : true;
    const isC_alimentationMatched = isC_alimentationCheckboxChecked
      ? [...checkedCheckboxesC_alimentation].some(
          (cb) => cb.id === C_alimentation
        )
      : true;
    const isA_typeMatched = isA_typeCheckboxChecked
      ? [...checkedCheckboxesA_type].some((cb) => cb.id === A_type)
      : true;
    // refroidissement
    const isRef_typeMatched = isRef_typeCheckboxChecked
      ? [...checkedCheckboxesRef_type].some((cb) => cb.id === Ref_type)
      : true;
    const isDebit_dairMatched = isDebit_dairCheckboxChecked
      ? [...checkedCheckboxesDebit_dair].some((cb) => cb.id === Debit_dair)
      : true;
    const issocketMatched = issocketCheckboxChecked
      ? [...checkedCheckboxessocket].some((cb) => cb.id === socket)
      : true;
    const isventilateurMatched = isventilateurCheckboxChecked
      ? [...checkedCheckboxesventilateur].some((cb) => cb.id === ventilateur)
      : true;
    ...
    article.style.display =
      ...
      isS_tailleMatched &&
      isS_typeMatched &&
      isB_dimensionMatched &&
      istaille_carte_mereMatched &&
      istaille_alimentationMatched &&
      istaille_carte_graphiqueMatched &&
      isA_dimentionMatched &&
      isC_alimentationMatched &&
      isA_typeMatched &&
      isRef_typeMatched &&
      isDebit_dairMatched &&
      issocketMatched &&
      isventilateurMatched &&
      isCoreMatched &&
      isC_carte_graphiqueMatched &&
      isf_processeurMatched &&
      isC_processeursMatched &&
      isf_RAMMatched &&
      isf_carte_mereMatched &&
      isf_stockageMatched
        ? "block"
        : "none";
  });
...
function filterArticlesByPrice() {
  ...
  document.querySelectorAll(".card").forEach((article) => {
    const prixArticle = parseFloat(article.dataset.prix);
    article.style.display =
      prixArticle >= minPrice && prixArticle <= maxPrice ? "block" : "none";
  });
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  filterCategories.forEach((filterCategory) => {
    ...
    selectBtn.addEventListener("click", function () {
      ...
      const originalHeight =
        aside === generalAside
          ? originalGeneralAsideHeight
          : originalAutreAsideHeight;
      aside.style.height =
        originalHeight + calculateVisibleOptionsHeight(aside) + "px";
  ...
  generalAside.style.height =
    originalGeneralAsideHeight +
    calculateVisibleOptionsHeight(generalAside) +
    "px";
  autreAside.style.height =
    originalAutreAsideHeight + calculateVisibleOptionsHeight(autreAside) + "px";
