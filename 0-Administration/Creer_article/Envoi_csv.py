#Si ca marche pas, ecrire dans cmd :
#takeown /f "E:\OneDrive\UwAmp\www\Commerce\0-Administration\Creer_article\Envoi_csv.py"
#icacls "E:\OneDrive\Site_web\UwAmp\www\Commerce\test\DL_image_online.py" /grant Users:RX
# Envoi_csv.py
...
ID = sys.argv[1]
type_article = sys.argv[2]
nom = sys.argv[3]
desc = sys.argv[4]
marque = sys.argv[5]
gamme = sys.argv[6]
quantite = sys.argv[7]
p_revient = sys.argv[8]
p_vente = sys.argv[9]
Critere_1 = sys.argv[10]
Critere_2 = sys.argv[11]
Critere_3 = sys.argv[12]
#print("arg crit3",Critere_3)
Critere_4 = sys.argv[13]
#print("arg crit4",Critere_4)
Critere_5 = sys.argv[14]
#print(Critere_5)


# Archive a oublié

header = "ID;type;nom;desc;marque;gamme;quantite;p_revient;p_vente;Critere_1;Critere_2;Critere_3;Critere_4;Critere_5\n"
ligne = f"{ID};{type_article};{nom};{desc};{marque};{gamme};{quantite};{p_revient};{p_vente};{Critere_1};{Critere_2};{Critere_3};{Critere_4};{Critere_5}\n"
#chemin_fichier = "C:\\Uwamp\\UwAmp\\www\\Commerce\\0-Administration\\Creer_article\\Article.csv"
chemin_fichier="E:\\OneDrive\\Site_web\\UwAmp\\www\\Commerce\\0-Administration\\Creer_article\\Article.csv"
#  E:\OneDrive\Site web\UwAmp\www\Commerce\0-Administration\Creer_article\Envoi_csv.py
print("\n  wwwwww",chemin_fichier,"\n")
'''
with open(chemin_fichier, "a+") as f:
    f.seek(0)  # Remet le curseur a la premiere ligne du tableau
    En_tete = f.readline()# Prend la premeire ligne du tableau
    if En_tete != header:
        f.seek(0)  # Remet le curseur a la premiere ligne du tableau
        f.write(header)# Print la ligne header
    f.write(ligne)#Print la ligne de donnée
'''
...
contenu_html = '''
    ...
    <html lang="en">
        ...
        $quer="SELECT * FROM article where Id_article='''+str(ID)+'''"; // Trie par ordre décroissant et prend QUE la première valeur
...
with open(repertoire, "w",encoding='utf-8') as fichier_html:
    fichier_html.write(contenu_html)
print(f"Fichier HTML créé avec succès à l'emplacement : {repertoire}")